...
# Thiết lập chuyển hướng lỗi
os.makedirs('logs', exist_ok=True)
# Chuyển hướng stderr của Python
sys.stderr = open('logs/stderr.log', 'w')  
# Chuyển hướng stdout của Python nếu muốn giữ log
...
def capture_image(filename="capture.jpg"):
    ...
    if not success:
        logger.error("Không lưu được ảnh!")
        return None

    # Kiểm tra file có thực sự tồn tại
    if not os.path.exists(filename):
        logger.error("File ảnh không tồn tại sau khi lưu!")
        return None

    logger.info(f"Đã lưu ảnh vào {filename}")
    ...
    while running:
        try:
            ...
            if tof is None:
                ...
                # logger.info("Đang khởi tạo cảm biến VL53L1X...")
                i2c = board.I2C()
                ...
                tof.start_ranging()
                logger.info("Cảm biến VL53L1X khởi tạo thành công, bắt đầu đo khoảng cách")
            ...
            if tof.data_ready:
                try:
                    ...
                    if distance is not None and distance >= 0:
                        if 100 <= distance <= 150:
                            print("Vật cản trong phạm vi 1 mét - 1,5 met t!")
                            voice_service.speak("Cảnh báo: Vật cản trong phạm vi 1 - 1,5 mét!")
                                ...
                                if image_file is None:
                                    ...
                                    try:
                                        with open(image_file, 'rb') as f:
                                            ...
                                            voice_service.speak(data.get("data", "Không phát hiện vật cản.").get("data", "Không có dữ liệu"))
...
def init_phan_doan_lan_duong(cap, camera_lock):
    logger.info("Khởi tạo phân phân đoạn làn đường")
    try:
        predict_camera(cap, camera_lock, running=True, frame_skip=2)
    except Exception as e:
        logger.error(f"Lỗi trong luồng phân đoạn làn đường: {e}")
    finally:
        # logger.info("Đã dừng luồng phân đoạn làn  đường")
        pass
...
def xu_ly_yeu_cau(voice_service, gps_service, api_service): 
    ...
    voice_service.speak("Đang lắng nghe yêu cầu của bạn. Bạn có thể nói bắt đầu bằng 'mi' để hỏi chatbot hoặc '3' để điều hướng.")
    try:
        while running:
            ...
            if text:
                ...
                if words:
                    ...
                    if first_word in ["mi", "me", "my"]:
                        ...
                        handle_navigtion(navigator, gps_service, textvoice)
                    else:
                        voice_service.speak("Yêu cầu không hợp lệ. Vui lòng thử lại.")
            voice_service.speak("Đang lắng nghe yêu cầu tiếp theo. Bạn có thể nói bắt đầu bằng 'mi' để hỏi chatbot hoặc '3' để điều hướng.")
    except Exception as e:
        logger.error(f"Lỗi trong luồng xử lý yêu cầu: {e}")
    finally:
        # logger.info("Đã dừng luồng xử lý yêu cầu")
        pass
